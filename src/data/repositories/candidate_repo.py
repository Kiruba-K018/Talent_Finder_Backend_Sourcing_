...
async def find_by_name_and_title(candidate_name: str, title: str) -> dict | None:
    ...
    try:
        return await _collection().find_one({"candidate_name": candidate_name, "title": title})
    except Exception as e:
        logger.error("failed_to_query_by_name_title", name=candidate_name, title=title, error=str(e))
        return None

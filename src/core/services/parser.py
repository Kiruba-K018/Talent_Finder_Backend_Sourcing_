...
def parse_name(soup: BeautifulSoup) -> str:
    ...
    for h1 in soup.find_all('h1'):
        name = _text(h1).strip()
        if name and len(name) > 1 and not any(x in name.lower() for x in ['follow', 'message', 'connect', 'open to', 'looking']):
            # Avoid button text
            if len(name.split()) <= 5:  # Names usually have 1-5 words
                return name
    
    # Strategy 3: Look for text near LinkedIn title (usually appears after title in header)
    title_section = soup.find('div', {'class': lambda x: x and 'pv-text-details__left-panel' in (x or '')})
    if title_section:
        # Get all text, split by newline, first non-empty line should be name
        lines = [l.strip() for l in _text(title_section).split('\n') if l.strip()]
    ...
    if header:
        for text_elem in header.find_all(['span', 'div']):
            ...
            if name and len(name) > 1 and 1 <= len(name.split()) <= 5:
                # Check it's not a button or action text
                if not any(x in name.lower() for x in ['follow', 'message', 'connect', 'open to', 'looking', 'download', 'share']):
                    return name
...
def parse_title(soup: BeautifulSoup) -> str:
    ...
    if header:
        ...
        if len(lines) >= 2:
            ...
            if len(potential_title) < 200 and len(potential_title.split()) <= 10:
                if not any(x in potential_title.lower() for x in ['follow', 'message', 'connect', 'download', 'email']):
    ...
    match = re.search(r'([A-Za-z\s&]+?)\s*(?:@|at)\s*([A-Za-z0-9&\s\.\-,]+?)(?:\n|$|·|\|)', all_text)
    if match:
        title = match.group(1).strip()
        if len(title) > 2 and len(title) < 150 and not any(x in title.lower() for x in ['message', 'follow', 'connect']):
            return title
    
    # Strategy 5: Look in experience section for first job title
    exp_section = soup.find('section', {'class': lambda x: x and 'experience' in (x or '').lower()})
    if exp_section:
        first_item = exp_section.find('li')
        if first_item:
            item_text = _text(first_item)
            title_match = re.search(r'\b([A-Za-z ]+(?:Engineer|Developer|Manager|Analyst|Designer|Lead|Architect|Officer))\b', item_text)
            if title_match:
...
def parse_location(soup: BeautifulSoup) -> str:
    ...
    if header:
        for elem in header.find_all(['span', 'div']):
            ...
            if ',' in text and len(text) < 100:
                # Capture location patterns like "City, Country" or "City, State, Country"
    ...
    if match:
        ...
        if len(potential_location) < 100 and not any(x in potential_location.lower() for x in ['message', 'connect', 'follow']):
...
def parse_summary(soup: BeautifulSoup) -> str:
    ...
    for elem in soup.find_all('div', {'class': lambda x: x and 'inline-show-more-text' in (x or '')}):
        ...
        if hidden_span:
            ...
            if summary and len(summary) > 30:
                ...
                logger.debug(f"Found summary via aria-hidden span: {len(summary)} chars")
                return summary[:2000]
        
        # Fallback: get from visually-hidden span
        visible_span = elem.find('span', {'class': lambda x: x and 'visually-hidden' in (x or '')})
        if visible_span:
            summary = _text(visible_span).strip()
            if summary and len(summary) > 30:
                summary = re.sub(r'\s+', ' ', summary)
                logger.debug(f"Found summary via visually-hidden span: {len(summary)} chars")
                return summary[:2000]
    
    # Strategy 2: Look for specific summary section IDs/classes
    for selector in ["section[id*='about']", "div[class*='about']", "section[class*='summary']", "div[data-section='about']"]:
        tag = soup.select_one(selector)
        if tag:
            summary = _text(tag).strip()
            if len(summary) > 50:
                summary = re.sub(r'\s+', ' ', summary)
                logger.debug(f"Found summary via selector '{selector}': {len(summary)} chars")
                return summary[:2000]
    ...
    for section in sections:
        ...
        if any(x in class_str.lower() for x in ['nav', 'header', 'footer', 'profile-card', 'title', 'headline']):
            continue
...
def extract_skills_from_text(text: str) -> list[str]:
    ...
    skills_keywords = [
        ...
        "TensorFlow", "PyTorch", "Pandas", "NumPy", "Scikit-learn", "Machine Learning", "AI", "NLP",
        "Agile", "Scrum", "DevOps", "Microservices", "SQL", "NoSQL", "CI/CD"
    ]
...
def parse_experience(soup: BeautifulSoup) -> list[dict]:
    ...
    try:
        ...
        items = soup.find_all('li', {'class': lambda x: x and 'artdeco-list__item' in (x or '')})
        ...
        for item in items[:20]:  # Limit to prevent spam
            try:
                ...
                has_date = bool(re.search(r'(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|present)',  item_text, re.IGNORECASE))
                has_employment_type = bool(re.search(r'\b(Full-time|Part-time|Contract|Temporary|Freelance|Internship)\b', item_text))
                has_job_title = bool(re.search(r'\b(Engineer|Developer|Manager|Analyst|Designer|Lead|Senior|Junior|Architect|Specialist|Officer|Admin)\b', item_text))
                
                if not (has_job_title or (has_date and has_employment_type)):
                    continue
                
                # Extract fields carefully and clean duplicates
                job_role_match = re.search(r'\b([A-Za-z ]+(?:Engineer|Developer|Manager|Analyst|Designer|Lead|Architect|Officer))\b', item_text)
                job_role = clean_duplicates(job_role_match.group(1).strip()) if job_role_match else ""
                
                company_match = re.search(r'(?:at|@)\s+([A-Z][A-Za-z0-9&\s\-\.]+?)(?:\n|$|·|–|\|)', item_text)
                company_name = clean_duplicates(company_match.group(1).strip()) if company_match else ""
                
                emp_type_match = re.search(r'\b(Full-time|Part-time|Contract|Internship|Temporary|Freelance)\b', item_text)
                job_type = emp_type_match.group(1) if emp_type_match else ""
                
                # Extract dates
                start_date = ""
                end_date = ""
                date_range = re.search(r'([A-Za-z]+\s+\d{4})\s*(?:–|-|to)\s*(Present|[A-Za-z]+\s+\d{4})', item_text)
                if date_range:
                ...
                if job_role or company_name:
                    ...
                    is_duplicate = any(
                        clean_duplicates(e["job_role"]) == clean_duplicates(entry["job_role"]) and 
                        clean_duplicates(e["company_name"]) == clean_duplicates(entry["company_name"])
                        for e in entries
                    )
                    if not is_duplicate:
...
def parse_education(soup: BeautifulSoup) -> list[dict]:
    ...
    try:
        ...
        for item in items[len([x for x in items if re.search(r'(Engineer|Developer|Manager|Lead)', _text(x))]):][:15]:  # Skip job items, limit education items
            try:
                item_text = _text(item).strip()
                if len(item_text) < 5:
                    continue
                ...
                course_name = course_match.group(1).strip() if course_match else "Not specified"
                ...
                if degree_type or school_name or course_name:
                    ...
                    # Deduplicate
                    is_duplicate = any(
                        e["degree"] == entry["degree"] and e["course"] == entry["course"]
                        for e in entries
                    )
                    if not is_duplicate:
                        entries.append(entry)
    ...
    try:
        ...
        for item in items:
            try:
                ...
                has_project_keyword = bool(re.search(
                ...
                is_job_only = bool(re.search(r'(Full-time|Part-time|Contract).*?(?:Engineer|Developer|Manager|Lead)', item_text, re.IGNORECASE))

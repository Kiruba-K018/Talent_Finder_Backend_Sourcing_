...
def parse_postjobfree_resume(raw_text: str) -> dict:
    ...
    try:
        if not raw_text or not isinstance(raw_text, str):
            logger.warning("empty_or_invalid_resume_text", text_type=type(raw_text).__name__)
            return {}
        ...
        logger.error("parse_postjobfree_resume_failed", error=str(e), exception_type=type(e).__name__)
        return {}
...
def _extract_name(text: str) -> Optional[str]:
    ...
    try:
        ...
        candidate_name_match = re.search(r'^CANDIDATE_NAME:\s*(.+)$', text, re.MULTILINE)
        if candidate_name_match:
            ...
            if name and len(name) > 2 and re.search(r'[a-zA-Z]', name):
                ...
                name = re.sub(r'\b(Jr|Sr|Mr|Ms|Mrs|Dr|Prof|MD|PhD)\b\.?', '', name, flags=re.IGNORECASE).strip()
        ...
        location_keywords = [
            'experience', 'education', 'skill', 'project', 'about', 'summary',
            'india', 'delhi', 'chennai', 'bangalore', 'mumbai', 'pune', 'hyderabad',
            'california', 'new york', 'texas', 'florida', 'usa', 'uk', 'canada',
            'australia', 'uae', 'singapore', 'london', 'dubai', 'sydney',
            'tamil nadu', 'maharashtra', 'karnataka', 'telangana',
            'pvt', 'ltd', 'inc', 'llc', 'corporation', 'company', 'co.',
            'at ', 'street', 'avenue', 'road', 'lane', 'drive', 'boulevard',
        ]
        ...
        for line in lines[:5]:
            ...
            name = re.sub(r'\b(Jr|Sr|Mr|Ms|Mrs|Dr|Prof|MD|PhD)\b\.?', '', line, flags=re.IGNORECASE).strip()
            
            # Validate name format (should have letters, preferably 1-2 words)
            if name and len(name) > 2 and re.search(r'[a-zA-Z]', name):
                # Final check: should not look like a location or company
                if ', ' not in name or not any(loc in name.lower() for loc in location_keywords):
...
def _extract_title(text: str) -> Optional[str]:
    ...
    try:
        ...
        lines = [l.strip() for l in text.split('\n')[:10] if l and l.strip()]
        for line in lines:
            if any(keyword in line.lower() for keyword in ['developer', 'engineer', 'manager', 'analyst', 'architect', 'specialist']):
                # Clean up the line
...
def _extract_summary(text: str) -> Optional[str]:
    ...
    try:
        ...
        if about_match:
            ...
            if sentences:
                return (sentences[0] + '.').strip() if not sentences[0].endswith('.') else sentences[0].strip()
...
def _extract_location(text: str) -> Optional[str]:
    ...
    try:
        ...
        location_pattern = r'([A-Za-z\s]+),\s*([A-Za-z\s]+),\s*(India|USA|UK|UAE|Canada|Australia)'
        ...
        lines = text.split('\n')
        for line in lines:
            if line and ('india' in line.lower() or 'delhi' in line.lower() or 'bangalore' in line.lower()):
                # Extract location from line
                location = re.search(r'([A-Za-z\s]+)\s*,\s*(India|Delhi|Bangalore|Mumbai|Chennai)', line, re.IGNORECASE)
                if location:
    ...
    try:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                if email.count('@') == 1 and '*' not in email:  # Prefer unmasked
                    return email
                elif '*' not in email:  # Return unmasked if available
                    return email
...
def _extract_hard_skills(text: str) -> list[str]:
    ...
    try:
        ...
        tech_keywords = [
            ...
            'FastAPI', 'Django', 'Flask', 'Spring Boot', 'NestJS', 'Laravel', 'React', 'Vue', 'Angular',
            'Node.js', 'Express', 'TypeScript', 'SQL', 'PostgreSQL', 'MySQL', 'MongoDB', 'Redis',
            'Docker', 'Kubernetes', 'AWS', 'GCP', 'Azure', 'Git', 'Jenkins', 'CI/CD',
            'REST API', 'GraphQL', 'Microservices', 'Linux', 'Windows', 'Terraform',
            'Prometheus', 'Grafana', 'Kafka', 'Airflow', 'Spark', 'Hadoop',
            'TensorFlow', 'PyTorch', 'Scikit-learn', 'NumPy', 'Pandas',
            'AI', 'Machine Learning', 'Deep Learning', 'NLP', 'Computer Vision',
            'Ansible', 'Helm', 'Nginx', 'Apache', 'Shopify', 'WordPress',
            'Bootstrap', 'Tailwind', 'HTML', 'CSS', 'XML', 'JSON',
            'Power BI', 'Jupyter', 'PyCharm', 'VS Code', 'Postman',
            'MVC', 'MVVM', 'API Integration', 'Payment Gateway', 'Third-party API'
        ]
...
def _extract_soft_skills(text: str) -> list[str]:
    ...
    try:
        ...
        soft_keywords = [
            ...
            'Critical Thinking', 'Mentorship', 'Negotiation', 'Presentation',
            'Attention to Detail', 'Adaptability', 'Accountability', 'Initiative',
            'Teamwork', 'Creativity', 'Decision Making', 'Work Ethic'
        ]
...
def _extract_experience(text: str) -> list[dict]:
    ...
    try:
        ...
        for line in lines:
            ...
            if any(keyword in line for keyword in ['Ltd', 'Inc', 'Pvt', 'LLC', 'Corporation', 'Co.']) or (
            ...
            if date_match:
                if 'company_name' in current_exp:
            ...
            if any(keyword in line for keyword in ['Developer', 'Engineer', 'Manager', 'Analyst', 'Lead', 'Senior', 'Junior']):
                if 'company_name' in current_exp and 'job_role' not in current_exp:
...
def _extract_projects(text: str) -> list[dict]:
    ...
    try:
        ...
        for block in project_blocks:
            ...
            tech_keywords = ['Python', 'Java', 'JavaScript', 'React', 'Django', 'FastAPI', 'PostgreSQL', 'AWS', 'Docker']
    ...
    try:
        ...
        for degree_pattern in degree_patterns:
            ...
            for match in matches:
                degree_match = re.search(r'(M\.Sc|MSc|M\.Tech|B\.Tech|B\.Sc|MBA|Diploma|Bachelor|Master)', match.group(0), re.IGNORECASE)
                course_match = match.group(1) if match.lastindex and match.lastindex >= 1 else ""

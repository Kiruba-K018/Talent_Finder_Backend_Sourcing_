...
def format_postjobfree_resume_from_html(html_content: str) -> dict:
    ...
    try:
        ...
        div_match = re.search(r'<div[^>]*class=["\']normalText["\'][^>]*>(.*?)</div>', html_content, re.DOTALL | re.IGNORECASE)
        if not div_match:
        ...
        logger.info(
            ...
            first_three=[cleaned_p_texts[i] if i < len(cleaned_p_texts) else None for i in range(3)]
        )
        ...
        logger.debug("structured_html_resume_data", data=json.dumps(structured_data, default=str)[:500])
...
def _format_with_llm_from_html_structure(structured_data: dict, original_html: str) -> dict:
    """
    ...
    try:
        ...
        if not settings.groq_api_key:
            logger.warning("groq_api_key_not_configured, using extracted HTML data as-is")
...
Return as JSON with complete structure matching all fields."""
        ...
        if not json_match:
            logger.warning("llm_html_parsing_invalid_json", response=response_text[:200])
            return _parse_html_structure_without_llm(structured_data)
        
        formatted = json.loads(json_match.group(0))
        logger.info("html_resume_formatted_with_llm", candidate_name=formatted.get("candidate_name"))
...
def _parse_html_structure_without_llm(structured_data: dict) -> dict:
    ...
    try:
        ...
        logger.debug("parsed_html_structure_without_llm", candidate_name=candidate_data.get("candidate_name"))
...
def _extract_text(text: str) -> Optional[str]:
...
def _extract_skills_from_text(text: str) -> list[str]:
    ...
    skills = []
    tech_keywords = [
        'Python', 'Java', 'JavaScript', 'C++', 'PHP', 'FastAPI', 'Django', 'Laravel',
        'React', 'Vue', 'Angular', 'Node.js', 'PostgreSQL', 'MySQL', 'MongoDB',
        'Docker', 'Kubernetes', 'AWS', 'Azure', 'GCP', 'Git', 'REST API', 'GraphQL',
        'Shopify', 'WordPress', 'NumPy', 'Pandas', 'Power BI'
    ]
...
def _extract_soft_skills_from_text(text: str) -> list[str]:
    ...
    soft_keywords = [
        ...
        'Agile', 'Scrum', 'Project Management', 'Mentorship'
    ]
...
def _extract_location_from_text(text: str) -> Optional[str]:
    ...
    location_pattern = r'\b(Noida|Delhi|Bangalore|Mumbai|Pune|Hyderabad|Chennai|India|UAE|Dubai)\b'
  ...
  "education": [
    {{
      ...
      "course": "string or null"
        ...
        # Call LLM
        logger.debug("calling_llm_for_resume_formatting", candidate_name=raw_candidate.get("candidate_name"))
        print("Raw candidate data:", json.dumps(raw_candidate, indent=2))
        response = llm.invoke([system_prompt, user_prompt])
        ...
        formatted = json.loads(json_match.group(0))
        logger.info("resume_formatted_with_llm", candidate_name=formatted.get("candidate_name"))
        print("Formatted candidate data:", json.dumps(formatted, indent=2))
        
        return _prepare_candidate_data(formatted)
        
    except Exception as e:
        logger.error("failed_to_format_with_llm", error=str(e), candidate_name=raw_candidate.get("candidate_name"))
...
def _prepare_projects(projects: list) -> list[dict]:
    ...
    for proj in projects:
        ...
        prepared.append({
            ...
            "technology_used": proj.get("technology_used", []) if isinstance(proj.get("technology_used"), list) else [],

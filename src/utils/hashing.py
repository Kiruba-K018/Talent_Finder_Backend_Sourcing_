...
def compute_identity_hash(
    name: str = None, email: str = None, location: str = None, profile_url: str = ""
) -> str:
...
def compute_profile_hash(
    ...
    skills_str = ",".join(
        sorted(s.lower() if isinstance(s, str) else str(s) for s in skills)
    )
    edu_str = str(sorted(str(e) for e in education))
    exp_str = str(sorted(str(e) for e in experience))
    certs_str = ",".join(
        sorted(c.lower() if isinstance(c, str) else str(c) for c in certifications)
    )
